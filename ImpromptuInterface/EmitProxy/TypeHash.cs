...
namespace ImpromptuInterface.Build
{
    ...
    public class TypeHash
    {
        ...
        public TypeHash(IEnumerable<Type> moreTypes)
            : this(false, moreTypes.SelectMany(r=>r.GetMembers()).ToArray())
        {
        ...
        public TypeHash(Type type1, params Type[] moreTypes)
            : this()
        {
            Types = type1.GetMembers().Concat(moreTypes.SelectMany(r=>r.GetMembers()).OrderBy(it => it.Name)).ToArray();
