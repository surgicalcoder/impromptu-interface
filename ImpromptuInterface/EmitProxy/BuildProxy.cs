...
namespace ImpromptuInterface.Build
{
    ...
    public static class BuildProxy
    {
        ...
        {  
              GenerateAssembly(name, AssemblyBuilderAccess.RunAndCollect,ref _tempSaveAssembly,ref  _tempBuilder);
        ...
        public static bool PreLoadProxy(Type proxyType, ActLikeProxyAttribute attribute = null)
        {
            var tSuccess = true;
            if (attribute == null)
                attribute = proxyType.GetTypeInfo().GetCustomAttributes(typeof(ActLikeProxyAttribute), inherit: false).Cast<ActLikeProxyAttribute>().FirstOrDefault();
        ...
        public static bool PreLoadProxiesFromAssembly(Assembly assembly)
        {
            ...
            var typesWithMyAttribute =
             ...
             let tAttributes = tType.GetTypeInfo().GetCustomAttributes(typeof(ActLikeProxyAttribute), inherit:false)
             where tAttributes != null && tAttributes.Count() == 1
        ...
        private static Type BuildTypeHelper(ModuleBuilder builder,Type contextType,params Type[] interfaces)
        {
            ...
            foreach (var tInterface in tInterfaces.Distinct())
            {
                ...
                {
                    var tAttrs = tInterface.GetTypeInfo().GetCustomAttributes(typeof(DefaultMemberAttribute), true);
                ...
                //if (tInterface != null && tAttr ==null)
                //    {
                //        var tCustomAttributes = tInterface.GetTypeInfo().GetCustomAttributes();
                ...
                var tNonRecursive = tInterface.GetTypeInfo().GetCustomAttributes(typeof(NonRecursiveInterfaceAttribute), true).Any();
        ...
        private static Type ReplaceTypeWithGenericBuilder(Type type, IDictionary<Type,GenericTypeParameterBuilder> dict)
        {
            ...
            if(tStartType.GetTypeInfo().IsGenericType)
            {
            ...
            }else if (dict.ContainsKey(tStartType))
                {
                    ...
                    var tAttributes = tStartType.GetTypeInfo().GenericParameterAttributes;
                    tNewType.SetGenericParameterAttributes(tAttributes);
                    foreach (var tConstraint in tStartType.GetTypeInfo().GetGenericParameterConstraints())
                    {
                        if (tConstraint.GetTypeInfo().IsInterface)
                            tNewType.SetInterfaceConstraints(tConstraint);
                        else
                            tNewType.SetBaseTypeConstraint(tConstraint);
                    }
                    tReturnType = tNewType.DeclaringType;
                }
                else
                {
        ...
        private static void MakeMethod(ModuleBuilder builder,MethodInfo info, TypeBuilder typeBuilder, Type contextType, bool nonRecursive= false, bool defaultImp =true)
        {
            ...
            if (info.GetCustomAttributes(typeof(UseNamedArgumentAttribute), false).Any() ||
                info.DeclaringType.GetTypeInfo().GetCustomAttributes(typeof(UseNamedArgumentAttribute), false).Any() )
            ...
            foreach (var tParam in info.GetParameters())
            {
                ...
                var tCustomAttributes = tParam.GetCustomAttributes();
                foreach (var tCustomAttribute in tCustomAttributes)
                {
                    try
                    {
                     //   tParamBuilder.SetCustomAttribute(GetAttributeBuilder(tCustomAttribute)); // TODO
                    }
        ...
        private static Tuple<Type, Type[]> GetParamTypes(dynamic builder, MethodInfo info)
        {
            ...
            if (tGenParams.Any())
            {
                ...
                {
                    returnType = tDict[returnType].DeclaringType;
                }
            ...
            )
        {
            ...
            var tRecurse = emitInfo.ResolveReturnType.GetTypeInfo().IsInterface && !emitInfo.NonRecursive;
        ...
        private static void MakeEvent(ModuleBuilder builder, EventInfo info, TypeBuilder typeBuilder, Type contextType, bool defaultImp)
        {
            ...
            tEmitInfo.CallSiteType = tCStp.CreateTypeInfo().AsType();
                                               ...
                                               FieldInfo tGetField, FieldInfo tSetField, FieldInfo tIsEventField, EmitEventInfo tEmitInfo)
        {
                ...
                ) //if IsEvent Not True
            {
                ...
                {
                    tIlGen.EmitDynamicBinaryOpBinder(CSharpBinderFlags.None, ExpressionType.AddAssign, tEmitInfo.ContextType, tEmitInfo.ResolveReturnType);
        ...
        private static void MakePropertyHelper(ModuleBuilder builder, TypeBuilder typeBuilder, PropertyEmitInfo emitInfo, PropertyInfo info = null, MethodInfo getMethod = null, MethodInfo setMethod = null)
        {
            ...
            emitInfo.CallSiteType = tCStp.CreateTypeInfo().AsType();
 ...
 emitInfo.ResolveReturnType, emitInfo.ResolvedIndexParamTypes);
        ...
        internal static Type GenerateCallSiteFuncType(IEnumerable<Type> argTypes, Type returnType, MethodInfo methodInfo =null, TypeBuilder builder =null)
        {
            ...
            var tList = new List<Type> { typeof(CallSite), typeof(object) };
            tList.AddRange(argTypes.Select(it => (it.GetTypeInfo().IsNotPublic && !it.IsByRef) ? typeof(object) : it));
            ...
            lock (DelegateCacheLock)
            {
                ...
                {
                    tHash = TypeHash.Create(strictOrder: true, moreTypes: tList.Concat(   new[] {returnType}).SelectMany(r=>r.GetMembers()).ToArray() );
                }
...
// ReSharper restore UnusedParameter.Local
        ...
        private static void GenerateAssembly(string name, AssemblyBuilderAccess access, ref AssemblyBuilder ab, ref ModuleBuilder mb )
        {
            ...
            #if !SILVERLIGHT
            if (access == AssemblyBuilderAccess.RunAndCollect || access == AssemblyBuilderAccess.Run)
                mb = ab.DefineDynamicModule(string.Format("{0}.dll", tName.Name));
